...
def tracker(time, x_init, y_init):
    # x_init and y_init are arrays of Np particles
    x, y = np.array(x_init), np.array(y_init)
    x_hist, y_hist = [x.copy()], [y.copy()]
    
    t = 0
    while t < time:
        # Interpolate all particles at once (points shape: Np x 2)
        u_vel = u_spline.ev(x % L, y % L)
        v_vel = v_spline.ev(x % L, y % L)
        ...
        t += dt_eta
        print(t)
    return np.array(x_hist).T, np.array(y_hist).T  # Returns shape (Np, TimeSteps)

def compute_particle_tracks(x_a, y_a, time, Np):
    x_plot, y_plot = tracker(time, x_a, y_a)
    return {"x_data_a": np.array(x_plot), "y_data_a": np.array(y_plot)}
...
def compute_perturbations(x_a, y_a, theta, epsilon_perturbation, time, Np, x_data_a, y_data_a):
    ...
    for i in range(len(epsilon_perturbation)):
        # 1. Vectorized calculation of perturbed starting positions for all particles
        x_p_init = x_a + (epsilon_perturbation[i] * np.cos(theta[i]))
        y_p_init = y_a + (epsilon_perturbation[i] * np.sin(theta[i]))

        # 2. Track all perturbed particles at once using your vectorized tracker
        # Returns shape (Np, TimeSteps)
        x_p_traj, y_p_traj = tracker(time, x_p_init, y_p_init)

        # 3. Vectorized R^2 calculation (Difference between baseline and perturbed)
        # Note: (Np, Time) - (Np, Time) results in a (Np, Time) array of squared distances
        dx = x_data_a - x_p_traj
        dy = y_data_a - y_p_traj
        
        # Calculate squared distance for every particle at every time step
        r_square_all_particles = dx**2 + dy**2
        
        # 4. Average across all particles (axis 0) to get mean R^2 for this epsilon
        r_square_mean = np.mean(r_square_all_particles, axis=0)

        # 5. Store results
        x_data_all.append(x_p_traj)
        y_data_all.append(y_p_traj)
        r_square_perturbation.append(r_square_mean)
        
        print(f"Computed Epsilon index {i}: {epsilon_perturbation[i] / dx}")
...
u_spline = RectBivariateSpline(x, y, u1)
v_spline = RectBivariateSpline(x, y, v1)
